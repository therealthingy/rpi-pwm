...
actor "Authenticated User" as g
package RPiFC {
  usecase "Retrieve system status" as UC1
  usecase "Retrieve system infos" as UC2
  usecase "Retrieve debugging data" as UC3
  usecase "Change configuration" as UC4
}
g --> UC1
g --> UC2
g --> UC3
...
-----
@startuml
Frontend -> Backend: HTTP: POST /fanCurves
...
@startuml
scale 530 height
Frontend -> Backend: HTTP: PUT /fanCurves/<name>
...
alt auth_success
  ...
  alt found
    ...
    Frontend <-- Backend: HTTP 200 OK

    Frontend -> Frontend: updateFanCurveChart(fanSeries)
    Frontend -> Frontend: showSuccessMsg(msg)
  else not_found
    Frontend <-- Backend: HTTP 404
...
@startuml
loop every 3 sec
  Frontend -> Backend: GET /debugging/logs
...
node "<<device>> RPiFC client" as device_client #lightyellow {
  node "<<execution environment>> Webbrowser" as device_client_webbrowser #aliceblue
}

node "<<device>> RPi" as device_server #lightyellow {
  node "<<execution environment>> Docker" #aliceblue {
